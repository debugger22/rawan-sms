...
parser = argparse.ArgumentParser(
    description="Sends sms non-interctively using indyarocks.com")
parser.add_argument(
    '-t',
    '--authfile',
    metavar='FILE',
    default=default_auth_file,
    help='Specify the authorization file mannually. By default it is ~/.sendsms.auth')
parser.add_argument(
    '-s',
    '--setup',
    action='store_true',
    default=False,
    help="Configure and generate the default authfile for the first time. Will change the existing authfile if existed")
parser.add_argument(
    '-q',
    '--quiet',
    action='store_true',
    default=False,
    help="Don't output anything. Overrides --debug below")
parser.add_argument(
    '-d',
    '--debug',
    action='store_true',
...
parser.add_argument(
    '-u',
    '--username',
    help="Supply username here. If supplied, you will be prompted for the password")
parser.add_argument(
    '--force-login',
    action='store_true',
    help="Force a new login, dumping old cookies and session information")
parser.add_argument(
    "to",
    nargs='?',
    default=None,
    help="The number to send the message to. Can be a nick as defined in the Phonebook section of authfile. Can be multiple recipients separated by dot (.) ")
parser.add_argument(
    "message",
    default=None,
    nargs='?',
    help="The message you want to send. Else will be read from stdin")
...
if args.setup:
    ...
    config.set(
        'Login',
        'username',
        raw_input("Enter your phone number registered at FullonSMS:"))
...
#   config.set('Login','name',raw_input("Enter your Name:"))
    ...
    if not args.to:
        logging.critical(
            "Cannot find the 'to' argument. You did not specified to whom the message has to be sent")
        parser.print_help()
        sys.exit(5)
    ...
    if config.has_option('Phonebook', name):
        try:
            to = str(
                config.getint(
                    'Phonebook',
                    name))  # this ensure that the file contains a valid int
        except ValueError:
            logging.critical(
                "Your phonebook contains a malformed entry for %s" %
                name)
            sys.exit(4)
        else:
            return to
...
for s in senders:
    ...
    # That means that the args.to was not a number, now try to check if it is
    # on Phonebook
        to = get_from_phonebook(s)
        if not to:
        #not exist in phonebook
            logging.critical(
                "The name %s you specified was not found in the Phone book." %
                args.to)
            sys.exit(4)
        else:
...
if(len(message) > 160):
    logging.critical(
        "Message length exceeded 160 chars by %i chars" %
        (len(message) - 160))
    sys.exit(4)
message = message[:160]
###
...
filecookiejar = cookielib.MozillaCookieJar(
    os.path.expanduser('~/.sendsms.cookies'))
try:
    filecookiejar.load(ignore_discard=True)
except:
    logging.debug(
        "Cannot open the cookie file. Check if it exists. Ignore if this is first time.")
...
if args.force_login:
    loginfo(
        "--force-login selected. Will not consider previous authentication sessions")
    try_with_previous = False
...
def login(uname, passwd):
    ...
    login_encode = urllib.urlencode(
        {'MobileNoLogin': uname, 'LoginPassword': passwd})
    ...
    logging.debug(
        "Sent Login information, got the following return URL: %s",
        f.geturl())
...
for n in senders_numbers:
    ...
    while True:
        ...
        if result == 'sent':
            ...
            loginfo(
                "SMS Not sent to %s. Can't figure out why. Perhaps try again later\n" %
                n)
            break
        elif result == 'login':
            try_with_previous = False
